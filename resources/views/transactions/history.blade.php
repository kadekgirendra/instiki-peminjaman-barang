            ...
            <div class="bg-surface rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <button type="button" @click="openId = openId === {{ $loanRequestId }} ? null : {{ $loanRequestId }}"
                    ...
                    <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shrink-0
                        ...
                        @if ($isRejected)
                            <svg class="w-5 h-5 sm:w-6 sm:h-6 text-danger" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                        @else
                            <svg class="w-5 h-5 sm:w-6 sm:h-6 text-success" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs sm:text-sm text-slate-500 mt-0.5">
                            {{ $firstItem->start_date->translatedFormat('j M') }} – {{ $firstItem->end_date->translatedFormat('j M Y') }}
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        @if ($isRejected)
                            <span class="inline-flex items-center gap-1.5 bg-danger/10 text-danger text-xs font-semibold px-3 py-1.5 rounded-full">
                                <span class="w-1.5 h-1.5 rounded-full bg-danger"></span>
                                Ditolak
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1.5 bg-success/10 text-success text-xs font-semibold px-3 py-1.5 rounded-full">
                        ...
                        @if (!$isRejected && $isLate)
                            <span class="hidden sm:inline-flex bg-warning/10 text-warning text-xs font-semibold px-3 py-1.5 rounded-full">
                                Telat
                            </span>
                        @endif

                        <svg class="w-4 h-4 text-slate-400 transition-transform duration-200"
                             :class="openId === {{ $loanRequestId }} ? 'rotate-180' : ''"
                ...
                <div x-show="openId === {{ $loanRequestId }}" x-collapse x-cloak
                    ...
                    @if (!$isRejected && $isLate)
                        <span class="sm:hidden inline-flex bg-warning/10 text-warning text-xs font-semibold px-3 py-1.5 rounded-full mb-3">
                            Telat mengembalikan
                        </span>
                    @endif
                    ...
                    <div class="space-y-2 mb-4">
                        @foreach ($group as $transaction)
                            <div class="flex items-center justify-between bg-white rounded-lg px-3 sm:px-4 py-2.5 border border-slate-100">
                                <div>
                                ...
                                <span class="text-xs sm:text-sm text-slate-500 font-medium">{{ $transaction->quantity }} unit</span>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                        <div class="bg-white rounded-lg p-3 border border-slate-100">
                            ...
                            <p class="text-sm font-semibold text-secondary">{{ $firstItem->start_date->translatedFormat('j M Y') }}</p>
                        </div>
                        <div class="bg-white rounded-lg p-3 border border-slate-100">
                            <p class="text-xs text-slate-500 mb-0.5">Batas Kembali</p>
                            <p class="text-sm font-semibold text-secondary">{{ $firstItem->end_date->translatedFormat('j M Y') }}</p>
                        </div>
                        @if ($firstItem->returned_at)
                            <div class="bg-white rounded-lg p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-0.5">Dikembalikan</p>
                                <p class="text-sm font-semibold text-secondary">{{ $firstItem->returned_at->translatedFormat('j M Y') }}</p>
                            </div>
                        @endif
                        @if ($totalFee > 0)
                            <div class="bg-danger/5 rounded-lg p-3 border border-danger/20">
                                <p class="text-xs text-danger/70 mb-0.5">Total Denda</p>
                                <p class="text-sm font-bold text-danger">Rp {{ number_format($totalFee, 0, ',', '.') }}</p>
                            </div>
                        @endif
                    </div>
